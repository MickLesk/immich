  ...
  <section>
    ...
    <div class="rounded-lg bg-gray-100 p-4 dark:bg-black/40 overflow-y-auto">
      <div class="flex flex-col">
        ...
        <div class="my-3">
          <SettingSwitch id="require-password" bind:checked={enablePassword} title={$t('require_password')} />
        </div>

        <div class="my-3">
          <SettingSwitch id="show-metadata" bind:checked={showMetadata} title={$t('show_metadata')} />
        </div>
          ...
          {#if editingLink}
            <p class="immich-form-label my-2">
              <SettingSwitch
                ...
                title={$t('change_expiration_time')}
              />
            </p>
          {:else}
