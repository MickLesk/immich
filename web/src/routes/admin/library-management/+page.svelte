<script lang="ts">
  ...
  const onDeleteLibraryClicked = async () => {
    ...
    if (
      selectedLibrary &&
      confirm(
        $t('page.admin.library_management.delete_confirmation', { values: { libraryName: selectedLibrary.name } }),
      ) == true
    ) {
...
{#if confirmDeleteLibrary}
  <ConfirmDialogue
    id="warning-modal"
    title={$t('common.warning_exclamation')}
    prompt={$t('page.admin.library_management.delete_confirmation_asset_count', {
      values: { deleteAssetCount: deleteAssetCount },
    })}
                  ...
                  {#if showContextMenu}
                    <Portal target="body">
                      <ContextMenu {...contextMenuPosition} on:outclick={() => onMenuExit()}>
                        <MenuOption
                          on:click={() => onRenameClicked()}
                          text={$t('page.admin.library_management.menu_rename')}
                        />
